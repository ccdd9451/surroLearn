...
def tensor_linear_interval_range(begin, end, step_amount, rectified=True):
    with tf.variable_scope("", reuse=True):
        global_step = tf.get_variable(name="global_step", dtype=tf.int32)
        global_step = tf.cast(global_step, tf.float32)
        lin = (global_step / step_amount) * (end - begin) + begin
        return tf.maximum(lin, 0) if rectified else lin
